...
- Frankfurter.app (ECB-kurser) för USD/SEK och EUR/SEK - uppdateras en
  gång per bankdag, vilket räcker gott för en ticker som inte behöver
  vara minutfärsk.
...
def fetch_fx(base: str, quote: str) -> tuple:
    today = fetch_json(f"https://api.frankfurter.app/latest?from={base}&to={quote}")
    price = today["rates"][quote]
    yesterday = (datetime.date.today() - datetime.timedelta(days=1)).isoformat()
    try:
        # Frankfurter snappar automatiskt till senaste bankdag om datumet
        # infaller på en helg.
        prev = fetch_json(f"https://api.frankfurter.app/{yesterday}?from={base}&to={quote}")
        baseline = prev["rates"][quote]
    except Exception:
        baseline = price
...
SOURCES = [
    ...
    ("USD/SEK", lambda: fetch_fx("USD", "SEK"), 2),
    ("EUR/SEK", lambda: fetch_fx("EUR", "SEK"), 2),
