...
| | |
|---|---|
| **Steg 1** | Nyhetsbrev via mail, **dagligen kl ~08:00** (beslutat 2026-09-21, ursprungsplanen var veckovis→dagligt) med *nya* artiklar på teman jag är intresserad av. **Pausat 2026-09-22:** Gmail-utkastet skapas inte längre - hemsidan (Steg 2) är den enda aktiva leveransvägen just nu, se `axel-news-brief`-jobbets SKILL.md |
| **Steg 2** | Interaktiv app eller hemsida för att djupdyka i nyheterna. **Nu den enda aktiva leveransvägen** (se pausnoten ovan) - fullt kopplad end-to-end sedan 2026-09-22 (GitHub Pages + Supabase + lösenordsskydd + riktig kuraterad data) |
...
| **Kurering** | Dynamiskt antal, uppdaterat 2026-09-22 (tidigare 15): upp till 10 nyheter per kategori, men färre om inget nytt hänt sen sist — inte en fast kvot som tvingar fram fyllnadsnyheter. Varje nyhet med en rad om varför jag får den. Bra att ha med alla källor (bredd, se avsnitt 6 "Kandidatkälla") |
...
| **5. Nyhetsinsamling (L) — ✅ v1 klar 2026-09-20** | Gratis RSS per kategori (Google Nyheter-sök, brett), gruppera rubriker om samma händelse (trendsignal, känt grov), ranka på intressematch × trend × färskhet, källdiversifiering, brus-exkludering, topp N (dynamiskt, upp till 10 - sänkt från 15, 2026-09-22) per kategori till `data/profile/candidates/{kategori}.json`. Internationellt komplement via `finance-news`-MCP (`merge_candidates.py`, kräver Claude interaktivt). **Ej klart:** "sortera bort det du redan läst" - kräver Fas 3:s lokala historikläsning, som inte finns än. Görs i Fas 3/6 istället | HTTP, RSS, rankning | Fungerande kandidatlistor för alla sex kategorier. Kända kvarstående brister: grov lokal dublettgruppering (Fas 6 gör det riktiga jobbet), Jobb/Skvaller har litet urval, tröskelvärden empiriska - inte långtidstestade |
...
| **Kurering** | Uppdaterat 2026-09-22 (tidigare 15): dynamiskt, upp till 10 objekt per kategori - färre om inget nytt hänt sen sist (inte en fast kvot). Vart och ett med en rad om varför du får det. Format-inspiration: Omni - korta rubriker/nyheter först, fördjupning (inkl. källor) per objekt på begäran | Schemalagd uppgift |
...
**Kollaborativ signal (ny, 2026-09-20, ✅ Hacker News-delen klar 2026-09-22).** Uttryckligt önskemål: inte bara fortsätta bekräfta det jag redan läser/tittar på (ren innehållsbaserad matchning), utan också vad "såna som jag" gillar - en kompletterande signal utöver den egna historiken. Trendsignalen ovan mäter *redaktionell täckning* (hur många medier skriver om det), inte *läsarpopularitet*.

Testade båda kandidaterna direkt 2026-09-22: **Hacker News officiella Firebase-API** fungerar perfekt, gratis, ingen nyckel - `src/fetch_hackernews.py` hämtar toppstories, filtrerar på AI-nyckelord (minst 100 poäng) och sparar med poäng/kommentarantal i `data/profile/candidates_raw/ai_internationellt.json`. Testat mot skarp data: 16 relevanta träffar (GPT-6, Claude Opus 5.5, agentiska verktyg m.m.), riktiga poäng från 109 till 1870. Bara meningsfullt för AI-kategorin - HN domineras av amerikansk tech/startup-kultur, inte relevant för Jobb/Politik/Skvaller.

**Reddit pausat:** publik JSON utan inloggning (`reddit.com/r/.../top.json`) gav `403 Forbidden` vid test - Reddit stängde ner den fria vägen 2023, kräver numera en registrerad OAuth-app (client id/secret) även för gratisnivån. Inte värt registreringskrånglet för det extra värde det ger utöver HN och den redan breda källbasen - kan återupptas senare om det visar sig behövas.
...
**WSJ-källan borttagen ur MCP-sökningen (beslut 2026-09-22).** Den skarpa körningen 2026-09-22 upptäckte att `wsj`-källan i `finance-news`-MCP:n gav träffar från januari 2025 utan att flagga dem som inaktuella - jämfört med dagens datum var det över åtta månader gammal data som annars hade smugit sig in som "ny" nyhet. SKILL.md instruerar nu explicit att INTE söka källan `wsj` (ft/bloomberg/marketwatch/seekingalpha opåverkade, fortsatt aktiva). Kan återinföras om källan visar sig pålitlig igen vid en framtida kontroll.
...
**Hastighetsanalys och research-princip (beslut 2026-09-22).** En körning tog onödigt lång tid eftersom varje enskild kandidat (~30 per körning) fick samma djupa efterforskning via `WebSearch`/`WebFetch` för att skriva `las_mer` - detta var den klart största tidsposten, plus ett antal garanterat felande `WebFetch`-försök mot `news.google.com/rss/articles/...`-omdirigeringslänkar (de går alltid via en consent-vägg). Åtgärdat: `fetch_trusted_international.py` och `fetch_newsletter_feeds.py` sparar nu källans egen RSS-`description`/`summary`/`content:encoded` (HTML-stripad) direkt i candidates_raw-JSON:en - för nyhetsbrev (WordPress-flöden) är det ofta HELA inlägget, vilket helt tar bort behovet av att slå upp artikeln på nytt för de källorna. Google Nyheter-sökta svenska kandidater (`fetch_candidates.py`) fick INTE samma ändring - deras RSS-`description` är bekräftat bara rubriken duplicerad som länk, ingen extra information att hämta där. Ny princip: research (`WebSearch`, aldrig `WebFetch` på Google Nyheter-länkar) görs bara för motargumentet, marknadsdata-korsreferenser och de 2-3 viktigaste nyheterna per kategori - resten byggs på titel + sparad `description`. Se `.claude/scheduled-tasks/axel-news-brief/SKILL.md` steg 2-3 och 6 för de uppdaterade arbetsinstruktionerna.
...
**Skvaller: höjt substanskrav (beslut 2026-09-22).** `config/rss_queries.yaml` söker bara på två namn ("Bianca Ingrosso", "Parnevik"), vilket gör att kategorin nästan uteslutande fångar mindre lifestyle-/varumärkesinnehåll om samma två familjer (klädkollektioner, gamla Instagram-hack-repriser, TV-jurydrama) snarare än den bredare "kändis-/kriminaljournalistik" som `interests.yaml` beskriver kategorin som. Feedback: dessa kändes inte "riktigt intressanta". Fixat på kureringsnivå (inte genom att bredda sökfrågorna, som riskerar att importera generellt brus): explicit substanskrav i steg 5 - hoppa över rena marknadsförings-/varumärkessamarbeten och gamla repriser utan ny händelse, inkludera bara genuint läsvärda brotts- eller kändishändelser. Hellre 0-2 nyheter än fyllnad, samma princip som redan gäller Jobb/AI.
...
**✅ Klart 2026-09-22 (Steg 6). Lösenordsskydd via Supabase Auth.** Inloggningsruta (e-post + lösenord, `signInWithPassword`) döljer hela sidan tills en session finns; sessionen persisteras av Supabase-klienten så man slipper logga in varje gång, med utloggningsknapp i footern. Kontot skapades av Axel själv direkt i Supabase Dashboard (Authentication → Users, "Auto Confirm User") - Claude hanterar aldrig lösenordet.

**Viktig begränsning att komma ihåg när riktig brevdata kopplas in:** login-gaten skyddar sidans UI/JS, INTE statiska filer i `docs/` - GitHub Pages serverar allt där oavsett inloggning (så `docs/ticker.json` är t.ex. fortsatt publikt, vilket är okej eftersom börskurser inte är känsliga). Den riktiga personliga brevdatan (`data/profile/brief/<datum>.json`, se Steg 2 ovan) får därför ALDRIG kopieras rakt av till en statisk fil i `docs/` - `varfor_viktigt`-texterna avslöjar intresseprofilen. Den måste istället läsas från en Supabase-tabell skyddad av en RLS-policy som kräver en inloggad (`authenticated`) session för `select`, inte bara `anon`-insert som `ratings`/`feedback` har idag.

**Två buggar hittade och fixade samma dag, båda samma buggklass:** `.ticker-bar[hidden]` och senare `.auth-gate[hidden]` hade egna `display: flex`-regler som vann över `[hidden]`-attributets `display: none`, eftersom författarstilar alltid slår user-agent-stilar oavsett specificitet - `hidden`-attributet sattes korrekt av JS men syntes inte visuellt. Åtgärd: en explicit `.klass[hidden] { display: none; }`-regel per komponent. Kom ihåg detta mönster för framtida `hidden`-styrda element som också sätter sin egen `display`.

**✅ Klart 2026-09-22. Riktig brevdata kopplad in.** Ny Supabase-tabell `brief_items` (schema i `config/brief_schema.md`) med RLS som bara tillåter `select` för `authenticated`-rollen - ingen `insert`-policy för klienter, skrivning sker enbart via `src/upload_brief.py` med service_role-nyckeln (sparad lokalt i `config/.supabase_service_key`, gitignorerad, aldrig delad med Claude). Skriptet rensar och skriver om hela tabellen vid varje körning - sajten visar bara senaste brevet, ingen historik i UI:t än. Nytt Steg 9 i kureringsjobbets instruktioner kör skriptet direkt efter `save_brief.py`. `docs/index.html` läser tabellen efter inloggning (`loadBriefData()`) och byter ut exempeldatan om raderna finns - annars ligger fallbacken kvar. Testat end-to-end lokalt (fallback-vägen fungerar felfritt); väntar på att det schemalagda jobbet faktiskt kör med de nya stegen för ett första skarpt test av hela kedjan.

**✅ Klart 2026-09-22 (utvecklingsordningens Steg 2). Fas 6 skriver strukturerad brief-JSON.** Det dagliga kureringsjobbet (`axel-news-brief`) skrev tidigare bara ett Gmail-utkast direkt från kureringen. Nytt Steg 7 i jobbets instruktioner låter Claude skriva kategorivisa utkast (`data/profile/brief_draft/<kategori>.json`) enligt `config/brief_schema.md`, och ett nytt Steg 8 kör `src/save_brief.py` (nytt skript, testat end-to-end) som mekaniskt räknar ut deterministiska `id`-fält (SHA-256 av `kallor[0].lank`, via `src/brief_schema.py`) och sätter `kategori` - Claude ska aldrig räkna hashar för hand. Resultatet skrivs till `data/profile/brief/<datum>.json`. **Medvetet INTE kopplat till den publika `docs/`-sajten än** - den filen ligger under `data/` (privat, git-ignorerad) och innehåller `varfor_viktigt`-texter som indirekt avslöjar Axels intresseprofil; att publicera den kräver lösenordsskyddet ovan, som inte finns än. `bild`-fältet sätts alltid till `null` i det här steget - bildberikning (`src/fetch_og_images.py`) är fortsatt inte inkopplad i huvudflödet (separat, redan dokumenterad begränsning).

**Börsticker (design-prototypen, beslutat 2026-09-21):** tre varianter testade. 1) Eget schemalagt Claude Code-jobb som skrev en JSON-fil några gånger/dag - fungerande, men beroende av att Axels dator och Claude Code är igång, precis som Fas 6-begränsningen. 2) TradingViews gratis klientsidan-widget, inbäddad direkt i sidan - uppdaterades i besökarens egen webbläsare utan server, men gick inte att styla likt WSJ-originalets smala rad (eget typsnitt/ikoner/"D"-märkning låsta i en iframe). **Landade i en tredje variant:** samma egna, WSJ-styliserade ticker-design som i (1), men datakällan (`src/fetch_ticker.py`, samma fria nyckelfria API:er: Yahoo, Frankfurter.app, CoinGecko) körs nu av ett GitHub Actions-schemalagt jobb (`.github/workflows/update-ticker.yml`, var 4:e timme) istället för ett lokalt Claude Code-jobb - på GitHub:s servrar, inte Axels dator. Sidans JS läser `docs/ticker.json` (samma ursprung, ingen CORS-fråga) och faller tillbaka till fröade/simulerade värden om filen inte är nåbar (Claude-artefaktens sandlåda, eller innan sajten är deployad). Symboler: OMXS30, H&M, USD/SEK, EUR/SEK, GULD, BTC.
...
**Nyhetskort v2 (design-prototypen, backlog-post tillagd 2026-09-21):** fyra ändringar av nyhetskorten i `docs/index.html`, Omni-inspirerat.

1. **✅ Klart 2026-09-22. Längre förstanivå-sammanfattning.** `sammanfattning`-fältet (visas innan man klickar) är nu ~3 meningar istället för en rad. Uppdaterat i `config/brief_schema.md`, exempeldatan i `docs/index.html` och kureringsinstruktionen i det schemalagda jobbet (`axel-news-brief`).
...
**All-In Podcast (önskad källa, tillagd 2026-09-22):** allin.com saknar eget RSS-flöde, men det riktiga poddflödet hittades via iTunes Search API (`feedUrl` för det officiella podcast-id:t, inte de ospårade sökträffarna). Vanlig RSS 2.0, verifierad fungera direkt (427 avsnitt, 3 nya senaste 3 dagarna vid testkörning). Lagd under `ekonomi` i `config/trusted_international_feeds.yaml` (VC/makro/business är showens kärna, även om enskilda avsnitt ofta lika gärna handlar om AI eller politik - flytta kategori om det visar sig missvisande i praktiken). Kureringen får bara avsnittstitel+länk att jobba med, ingen transkription - samma begränsning som redan gäller nyhetsbreven.
