...
| **6. Brevet (M) — 🟡 v1 skapad 2026-09-20, ej testkörd** | Schemalagd uppgift (`axel-news-brief`) som läser projektmappen direkt (ingen Drive behövs, se rättelsen i avsnitt 2), kör Fas 5, applicerar all kurering (dublett/källvinkel/motargument/nyhetsbrev), skapar Gmail-utkast. Skapad utan schema - manuell körning först för att testa innan automatisering | Promptdesign, kopplingar | Ett brev anländer som utkast. Kvar: köra den första gången och granska resultatet, sedan sätta ett cron-schema |
...
**Nyhetsbrev via Gmail, "sedan sist"-mekanism (beslut 2026-09-20).** Fas 6 (schemalagd Claude-uppgift, oavsett kadens - dagligen eller veckovis) söker i Gmail efter mejl från kända nyhetsbrevsavsändare (`config/interests.yaml` → `nyhetsbrev_personer`, just nu Fernando del Pino Calvo-Sotelo och Kortsikt) **som kommit in sedan förra körningen**, inte allt från början varje gång. Kräver en sparad tidsstämpel ("senast kollad") - samma idempotens-princip som redan gäller Fas 3:s veckoskript (avsnitt 5, "Idempotens"). Gmail-verktyget är liksom `finance-news` bara tillgängligt för Claude, inte ett fristående skript - sker alltså i Fas 6, inte Fas 5. Inte byggt än, bara designbeslutet bekräftat.
